...
def compute_scores(
    ...
    *,
    ...
    pillar_weights: Mapping[str, float] | None = None,
    ...
    # Allow runtime overrides of pillar weights without mutating the base config
    if pillar_weights:
        pillars_cfg = cfg.setdefault("pillars", {})
        for key in ("alpha", "beta", "gamma"):
            if key in pillar_weights:
                pillars_cfg[key] = float(pillar_weights[key])
    ...
    if include_pillars:
        out["exposure_score"] = E
        out["sensitivity_score"] = S
        out["adaptive_capacity_score"] = AC
